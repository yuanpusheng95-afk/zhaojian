...
import { getFact, setFact } from './facts.js';
import { insertLane, readLaneLeaf, readLanes, recordLaneMove } from './lanes.js';
import { findEntries, findEntriesOnBranch } from './queries.js';
import { SESSION_TABLES, claimId } from './schema.js';
import {
  deleteSession,
  insertSession,
  listSessions,
...
export function createPostgresSessionRepo({ pool }: { pool: Pool }) {
  ...
  async function inTransaction<T>(fn: (client: PoolClient) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if ((error as any)?.name === 'SessionError') throw error;
      if (isUniqueViolation(error)) {
        throw sessionError('already_exists', (error as Error).message, error);
      }
  ...
  function openSession(id: string) {
    return new Session(createPostgresSessionStorage({ pool, sessionId: id }) as any);
}

  async function requireSession(client: PoolClient, id: string) {
  ...
  return {
     ...
     */
    async fork(source: any, options: any = {}) {

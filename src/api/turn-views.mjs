import { resolveAssetStorageKey } from '../infrastructure/storage/asset-storage.mjs';
...
async function buildCandidate(candidate, repository, assetStorage, ttlSeconds) {
  const view = {
    id: candidate.id,
  ...
  try {
    ...
    view.url = await assetStorage.getSignedUrl(key, {
      expiresInSeconds: ttlSeconds,
    });
    view.contentType = asset.metadata?.contentType ?? null;
  } catch (error) {
    if (error?.code !== 'ASSET_NOT_FOUND') throw error;
    view.urlError = error?.message ?? String(error);
  }
  return view;
}

export function createTurnViews({
  pool,
  repository,
  assetStorage,
  signedUrlTtlSeconds,
  ...
  async function loadTurnDetail({ projectId, turnId }) {
  ...
  async function fingerprint({ projectId, turnId }) {
    ...
    return row ? JSON.stringify([
      row.turn_updated_at,
      row.generations_updated_at,
      row.generation_count,
      row.selected_count,
    ]) : null;
  }

  async function turnChangedSince({ projectId, turnId, lastFingerprint }) {
  ...
  async function assertTurnExists({ projectId, turnId }) {
    const result = await pool.query(
      'SELECT 1 FROM agent_turns WHERE id = $1 AND project_id = $2',
      [turnId, projectId],
    );
...
function turnNotFound(projectId, turnId) {
  const error = new Error(`Turn not found in project ${projectId}: ${turnId}`);

...
export class AgentTurnWorker {
  ...
  constructor({ queue, runTurn, config, timers = { setTimeout, clearTimeout } }: {
    queue: any;
    runTurn: (turn: ClaimedTurn, signal: AbortSignal) => Promise<any>;
    config: WorkerConfig;
    timers?: TimerSet;
  }) {
  ...
  async runOnce(): Promise<ClaimedTurn | null> {
